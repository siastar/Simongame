...
function generateMainSequence() {
    ...
    for (i = 1; i <= Goal; i++) { // lenght of the Main sequence (it varies with level progress)
        ...
        if (1 <= myRnd && myRnd <= 25) {
            theNote = "A";
            theFreq = FreqA;
        } //evaluate the number and assign it a value ABCD
        else if (26 <= myRnd && myRnd <= 50) {
            theNote = "B";
            theFreq = FreqB;
        } //evaluate the number and assign it a value ABCD
        else if (51 <= myRnd && myRnd <= 75) {
            theNote = "C";
            theFreq = FreqC;
        } //evaluate the number and assign it a value ABCD
        else if (76 <= myRnd && myRnd <= 100) {
            theNote = "D";
            theFreq = FreqD;
        } //evaluate the number and assign it a value ABCD
        else {
            console.log("something goes wrong when myRnd is " + myRnd);
        } //say you to fuck offand check the myRnd value
...
function keyPress(event) { //event.target.preventDefault();
    ...
    else if (keyCode == 101 || keyCode == 97) {
        Played = "C";
        Freq = FreqC;
        userDidHisThing(Freq, Played);
    } // keyboard e a
    else if (keyCode == 114 || keyCode == 115) {
        Played = "D";
        Freq = FreqD;
        userDidHisThing(Freq, Played);
    } // keyboard r s
...
function changeButtonColor(frequency, time) {
    setTimeout(function() {
        if (frequency == FreqA) {
            ...
            document.getElementById("playA").style.background = 'black';
            resetButtonColor();
        } else if (frequency == FreqB) {
            console.log("RED BUTTON");
            document.getElementById("playB").style.background = 'black';
            resetButtonColor();
        } else if (frequency == FreqC) {
            console.log("YELLOW BUTTON");
            document.getElementById("playC").style.background = 'black';
            resetButtonColor();
        } else if (frequency == FreqD) {
            console.log("BLUE BUTTON");
            document.getElementById("playD").style.background = 'black';
            resetButtonColor();
        } else {
            console.log("Buttons color stuff");
        }
...
function checkIfCorrect() {
    ...
    stepCounter++;
    console.log(theMainSequence);
    if (thePartialSequence == []) {
        alert("Press Start");
    } else if (thePlayerSequence[index] == thePartialSequence[index] && stepCounter == Limit && Limit == Goal) {
        setTimeout(nextLevel, 1000);
    } // check if the level is complete and it calls nextLevel function																		
    else if (thePlayerSequence[index] == thePartialSequence[index] && stepCounter == Limit) {
        addAnother();
    } // check if the lpartial sequence has been completed and adds a new note to play																											
    else if (thePlayerSequence[index] == thePartialSequence[index]) {
        console.log("good job");
    } //check if the just played note is correct																										 		 																												  																								  																												      }													
    else {
        console.log("bad job");
...
function addAnother() { //console.log("things getting harder")
    stepCounter = 0;
    Limit++; // it adds a note to the sequence to repeat
